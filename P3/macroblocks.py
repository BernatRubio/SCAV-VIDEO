...
class Macroblocks:
    ...
    def showMacroblocks(self, input_file):
        ...
        cmd = (
            f'ffmpeg -hide_banner -flags2 +export_mvs -i {input_file} '
            f'-vf codecview=mv=pf+bf+bb ./{base_name}_macroblocks{ext}'
        )
    ...
    def createContainer(self, input_file):
        ...
        cmd = (
            f'ffmpeg -hide_banner -i {input_file} -ss 00:00:00 -t 00:00:50 '
            f'-c:v copy -c:a copy {output1}'
        )
        subprocess.run(cmd, shell=True, check=True)
        ...
        cmd5 = (
            f'ffmpeg -i {output1} -i {base_name}_mono.mp3 -i {base_name}_stereo.mp3 '
            f'-i {base_name}_stereo.aac -map 0:v:0 -map 1:a:0 -map 2:a:0 -map 3:a:0 '
            f'-metadata:s:a:0 title="MP3 Mono" -metadata:s:a:1 title="MP3 Stereo" '
            f'-metadata:s:a:2 title="AAC Stereo" -c:v copy -c:a copy output_with_3_audios{ext}'
        )
    ...
    def showStreams(self, input_file):
        cmd = (
            f'ffprobe {input_file} -hide_banner -show_entries format=nb_streams '
