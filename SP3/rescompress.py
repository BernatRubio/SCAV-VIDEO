...
class ResCompressor:
    ...
    def resize(self,input_file, resx, resy, output_dir):
        base_name, ext = os.path.splitext(os.path.basename(input_file))
        output_path = f"{base_name}_{resx}_{resy}{ext}"

        cmd = f'ffmpeg -hide_banner -i "{input_file}" -vf scale={resx}:{resy} "{output_dir}/{output_path}"'
        try:
    ...
    def changeCodec(self,input_file, codec, output_dir):
        base_name, ext = os.path.splitext(os.path.basename(input_file))
        if codec.lower() == "vp8":
            cmd = f'ffmpeg -i "{input_file}" -c:v libvpx -b:v 0 {output_dir}/{base_name}_{codec}.webm'
        elif codec.lower() == "vp9":
            cmd = f'ffmpeg -i "{input_file}" -c:v libvpx-vp9 -b:v 0 {output_dir}/{base_name}_{codec}.webm'
        elif codec.lower() == "h265":
            cmd = f'ffmpeg -i "{input_file}" -c:v libx265 -b:v 0 {output_dir}/{base_name}_{codec}.mp4'
        elif codec.lower() == "av1":
            cmd = f'ffmpeg -i "{input_file}" -c:v libaom-av1 -b:v 0 {output_dir}/{base_name}_{codec}.mkv'
    ...
    def compareCodecs(self,input_file,codec1, codec2, output_dir):
        ...
        cmd2 = f"ffplay {output_dir}/comparison_output.mp4"
